...
export default function CadastroPage() {
  ...
  async function handleCadastro(e: React.FormEvent) {
    ...
    // Atualiza perfil com CRECI e telefone
    const { data: { user } } = await supabase.auth.getUser()
    if (user) {
      await supabase.from('profiles').update({ creci, phone }).eq('id', user.id)
    }
      ...
      <div className="w-full max-w-sm">
        ...
        <form onSubmit={handleCadastro} className="bg-white rounded-2xl shadow-sm border border-stone-200 p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">CRECI <span className="text-stone-400 font-normal">(opcional)</span></label>
              <input
                type="text"
                value={creci}
            ...
            <div>
              <label className="block text-sm font-medium text-stone-700 mb-1">WhatsApp <span className="text-stone-400 font-normal">(opcional)</span></label>
              <input
                type="tel"
                value={phone}
